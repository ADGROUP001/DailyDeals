...
<!--            <script>
            ...
            cart.forEach(item => {
                cartItems.append(`
                    <li>
                        <img src="${item.imageUrl}" alt="${item.name}" style="width:70px; height:50px;">
                        ${item.name} - ₹${item.price} x ${item.quantity}
                        <button class="update-quantity" data-id="${item.id}">+</button>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.add-to-cart', function () {
                ...
                updateCart();
                updateCartInDatabase('add', {productID: productId, quantity: 1});
            });
            ...
            $(document).on('click', '.update-quantity', function () {
                ...
                cart.forEach(item => {
                    if (item.id === productId) {
                        item.quantity += 1;
                        updateCartInDatabase('update', {cartID: item.cartID, productID: productId, quantity: item.quantity});
                    }
                });
                ...
                cart.forEach(item => {
                    if (item.id === productId) {
                        if (item.quantity > 1) {
                            ...
                            cart = cart.filter(i => i.id !== productId);
                            updateCartInDatabase('remove', {cartID: item.cartID, productID: productId});
                        }
                    }
                });
            ...
            $('#cancel-cart').click(function () {
                ...
                updateCart();
                $('#cart').hide();
                updateCartInDatabase('clear', {cartID: cart[0]?.cartID});
            });

            // Load products on page load
            $.get('load_products.php', function(data) {
...
$(document).ready(function () {
    ...
    $('.add-to-cart').on('click', function () {
        ...
        $.ajax({
            url: 'cart.php',
            method: 'POST',
            data: { action: 'add', id: productId, name: productName, price: productPrice, image: productImage },
            dataType: 'json',
            success: function (response) {
                if (response.success) {
    ...
    $('#cart-items').on('change', '.item-quantity', function () {
        ...
        $.ajax({
            url: 'cart.php',
            method: 'POST',
            data: { action: 'update', id: cartItemId, quantity: newQuantity },
            dataType: 'json',
            success: function (response) {
                $('#cart-items').html(response.cartItems);
                $('#cart-total').text('₹ ' + response.cartTotal.toFixed(2));
    ...
    $('#cart-items').on('click', '.remove-cart-item', function () {
        ...
        $.ajax({
            url: 'cart.php',
            method: 'POST',
            data: { action: 'delete', id: cartItemId },
            dataType: 'json',
            success: function (response) {
                $('#cart-items').html(response.cartItems);
                $('#cart-total').text('₹ ' + response.cartTotal.toFixed(2));
